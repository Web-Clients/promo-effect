    ...
    try {
      ...
      for (const email of emails) {
        try {
          ...
          logger.error(`[Email Fetch] Failed to queue: ${queueError.message}`);
        }
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Fetch and process error:', error);
      return res.status(500).json({ error: error.message });
    }
  }
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Email process error:', error);
      return res.status(500).json({ error: error.message });
    }
  }
);
...
router.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      logger.error('Recent containers error:', error);
      return res.status(500).json({ error: error.message });
    }
  }
);
