...
const EURASIA_PATH = "M180,150 Q220,120 280,130 T400,160 Q450,140 500,120 T650,110 Q750,100 850,150 T950,220 L950,350 Q900,380 800,360 T650,320 Q550,300 450,330 T250,350 Q150,320 100,280 T150,200 Z";

const HUD_CORNER = (className: string) => (
  <div className={`absolute w-8 h-8 border-primary-500/40 pointer-events-none ${className}`} />
);

const PortNode = ({ x, y, label, data, code }: any) => {
  ...
  return (
    ...
    >
      ...
      <motion.circle 
        cx={x} cy={y} r="18" 
        stroke="#2563eb" strokeWidth="0.5" strokeOpacity="0.2" fill="none"
      ...
      {/* Label & Coordinates */}
      <text x={x + 15} y={y - 5} className="text-[10px] font-black uppercase tracking-tighter fill-white drop-shadow-md">
        {label}
      </text>
      <text x={x + 15} y={y + 8} className="text-[7px] font-bold uppercase tracking-widest fill-neutral-500">
        {code} | 31.23°N
      </text>
      ...
      <AnimatePresence>
        {isHovered && (
          ...
          >
            <rect x={x + 12} y={y - 80} width="160" height="70" rx="2" fill="black" fillOpacity="0.95" stroke="#2563eb" strokeWidth="1" strokeOpacity="0.3" />
            <path d={`M${x},${y} L${x+12},${y-10}`} stroke="#2563eb" strokeWidth="0.5" strokeDasharray="2 2" />
            
            <text x={x + 22} y={y - 62} className="text-[9px] font-black fill-primary-500 uppercase tracking-widest">LIVE VESSEL FEED</text>
            <rect x={x + 22} y={y - 55} width="140" height="0.5" fill="white" fillOpacity="0.1" />
            
            <text x={x + 22} y={y - 42} className="text-[8px] font-bold fill-neutral-300 uppercase">LOAD FACTOR: 94%</text>
            <text x={x + 22} y={y - 30} className="text-[8px] font-bold fill-neutral-300 uppercase">TRANSIT: 21 DAYS</text>
            <text x={x + 22} y={y - 18} className="text-[8px] font-bold fill-neutral-500 uppercase italic">Vessel: PROMO-MAERSK VI</text>
            
            <motion.rect 
              x={x + 140} y={y - 65} width="20" height="3" fill="#2563eb" 
...
const VesselTracker = ({ path, delay, vesselName }: { path: string, delay: number, vesselName: string }) => (
  <motion.g
  ...
  >
    ...
    <motion.circle 
      r="6" stroke="#60a5fa" strokeWidth="0.5" 
      animate={{ r: [6, 12], opacity: [0.5, 0] }}
      transition={{ repeat: Infinity, duration: 2 }}
    />
    <text y="-10" className="text-[6px] font-black fill-primary-500 uppercase tracking-widest pointer-events-none">{vesselName}</text>
  </motion.g>
);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <svg className="w-full h-full relative z-10" viewBox="0 0 1000 500" fill="none" xmlns="http://www.w3.org/2000/svg">
           {/* Detailed Coordinate Grid */}
           <defs>
              <pattern id="grid" width="100" height="100" patternUnits="userSpaceOnUse">
                 <path d="M 100 0 L 0 0 0 100" fill="none" stroke="white" strokeOpacity="0.03" strokeWidth="0.5"/>
              </pattern>
           </defs>
           ...
           {/* Lat/Long Labels */}
           <text x="50" y="20" className="text-[6px] fill-neutral-600 font-black tracking-widest uppercase">COORD: 44.4268° N, 26.1025° E</text>
           <text x="900" y="480" className="text-[6px] fill-neutral-600 font-black tracking-widest uppercase align-right">SYS_CLOCK: 12:55:03_UTC</text>

           {/* Animated Trade Paths */}
           {routes.map((path, i) => (
             <React.Fragment key={i}>
                <path d={path} stroke="#2563eb" strokeWidth="0.5" strokeOpacity="0.1" fill="none" strokeDasharray="5 5" />
        ...
        <div className="absolute top-8 left-8 w-48 p-4 bg-black/80 border-l border-primary-500/50 backdrop-blur-md">
           <div className="flex justify-between items-center mb-4">
              <span className="text-[8px] font-black text-white uppercase tracking-widest">Global Status</span>
           ...
           <div className="space-y-4">
              {[
                ...
                <div key={i}>
                   <div className="text-[6px] text-neutral-500 uppercase tracking-widest leading-none mb-1">{item.label}</div>
        ...
        <div className="absolute bottom-8 right-8 w-64 p-4 bg-black/80 border-r border-primary-500/50 backdrop-blur-md">
           <div className="text-[7px] text-primary-500 font-black mb-3 uppercase tracking-[0.3em]">Neural Route Optimizer v2</div>
           <div className="text-[6px] text-neutral-500 font-sans leading-relaxed uppercase">
              {">>"} CALCULATING OPTIMAL PATHS FOR Q2_2026<br/>
              {">>"} WEATHER_ANOMALY DETECTED IN SOUTH CHINA SEA<br/>
              {">>"} RE-ROUTING 4 VESSELS THROUGH MALACCA_STRAIT<br/>
