...
const SHIPPING_LINES = ['MSC', 'COSCO', 'Maersk', 'CMA CGM', 'Hapag-Lloyd', 'ONE', 'Evergreen', 'Yang Ming', 'HMM', 'Zim'];
const CONTAINER_TYPES = ['20DC', '40DC', '40HC', '20RF', '40RF'];

// Icons
const ShipIcon = () => (
  <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 17h18M3 17l2-9h14l2 9M8 8V5a1 1 0 011-1h6a1 1 0 011 1v3" />
  </svg>
);
const PlusIcon = () => (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-neutral-800 rounded-xl p-4 border border-neutral-200 dark:border-neutral-700">
          <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Linii Maritime</p>
          <p className="text-2xl font-bold text-primary-700 dark:text-primary-400 mt-1">{uniqueLines}</p>
        </div>
        <div className="bg-white dark:bg-neutral-800 rounded-xl p-4 border border-neutral-200 dark:border-neutral-700">
          <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Configurații Active</p>
          <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{activeConfigs}</p>
        </div>
        <div className="bg-white dark:bg-neutral-800 rounded-xl p-4 border border-neutral-200 dark:border-neutral-700">
          <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">Total Configurații</p>
          <p className="text-2xl font-bold text-neutral-700 dark:text-neutral-300 mt-1">{totalConfigs}</p>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Linie Maritimă</label>
              <select
            ...
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Taxe Locale Constanța ($)</label>
              <input
                type="number"
                step="0.01"
                min="0"
                value={formData.portTaxes}
                onChange={e => setFormData({ ...formData, portTaxes: parseFloat(e.target.value) || 0 })}
                className="w-full px-3 py-2 bg-white dark:bg-neutral-700 border border-neutral-300 dark:border-neutral-600 rounded-lg text-sm focus:ring-2 focus:ring-primary-500"
                required
              />
            </div>
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-neutral-600 dark:text-neutral-400">Filtrare după linie:</label>
        <select
          value={filterLine}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-neutral-500 dark:text-neutral-400">
          ...
          <p className="text-sm">Adăugați o linie maritimă cu tipuri de containere și taxe locale</p>
        </div>
      ) : (
        <div className="space-y-4">
          {(Object.entries(grouped) as [string, ShippingLineContainer[]][]).sort(([a], [b]) => a.localeCompare(b)).map(([line, lineItems]) => (
            <div key={line} className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
              {/* Line header */}
              <div className="px-5 py-3 bg-neutral-50 dark:bg-neutral-750 border-b border-neutral-200 dark:border-neutral-700 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-base font-semibold text-primary-800 dark:text-white">{line}</h3>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {lineItems.sort((a, b) => a.containerType.localeCompare(b.containerType)).map(item => (
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
        ...
        <p className="text-xs text-blue-700 dark:text-blue-400 leading-relaxed">
          ...
          Pentru fiecare linie maritimă și tip de container, specificați taxele portuare locale din Constanța.
          Dacă nu există o configurație specifică, calculatorul va folosi valoarea implicită din setările globale.
        </p>
      </div>
    </div>
  );
