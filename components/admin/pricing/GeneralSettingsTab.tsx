...
const defaultFormData = (): AdminSettingsInput => ({
  ...
  weightRanges: '[]',
  commissionPercentByIncoterm: { FOB: 10, EXW: 10, CFR: 10, CIF: 10 },
});

const INCOTERM_ORDER = ['FOB', 'EXW', 'CFR', 'CIF'] as const;
  ...
  useEffect(() => {
    if (settings) {
      setFormData({
        ...
        commissionPercentByIncoterm: settings.commissionPercentByIncoterm || {
          FOB: 10,
          EXW: 10,
          CFR: 10,
          CIF: 10,
        ...
        <div className="border-b pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Comision expediție (% pe Incoterm)
              </label>
              <p className="text-xs text-gray-500 mb-2">
                Se aplică la taxele locale + transportul terestru. Niciodată la navlul
                maritim — de asta la FOB comisionul nu mai crește odată cu tariful navei.
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {INCOTERM_ORDER.map((incoterm) => (
                  <div key={incoterm}>
                    ...
                    <div className="relative">
                      <input
                        ...
                        max={30}
                        step="0.1"
                        value={formData.commissionPercentByIncoterm?.[incoterm] ?? 10}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            commissionPercentByIncoterm: {
                              ...(formData.commissionPercentByIncoterm || {}),
                              [incoterm]: parseFloat(e.target.value) || 0,
                            },
                          })
                        }
                        className="w-full border border-gray-300 rounded-lg px-3 py-2 pr-7 focus:ring-blue-500 focus:border-blue-500"
                      />
